...
println("Step 1: A simple for loop from 1 to 5 inclusive")
 ...
 println("\nStep 3: Filter values using if condition in the loop")
   val donutIngredients = List("flour", "sugar", "egg yolks", "syrup", "flavoring", "caramel")
   for(ingredient <- donutIngredients if ingredient =="sugar"){
    println(s"Found sweetening ingredient = $ingredient")
  }
 println("\nStep 4: Filter values usiing if conditions in for loop and return the result back using the yield keyword")
   val sweeteningIngredients = for {
    ingredient <- donutIngredients
    if (ingredient == "sugar" || ingredient == "syrup"||ingredient=="caramel")
    } yield ingredient
     println(s"Sweetening ingredients are = $sweeteningIngredients")
